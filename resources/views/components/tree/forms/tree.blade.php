...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div {{ $attributes->merge($getExtraAttributes())->class([
        ...
        <div x-cloak class="flex gap-2 mb-2">
            <x-filament::link tag="button" size="sm" x-on:click="toggleSelectAllCheckboxes()">
                <span x-show="areAllCheckboxesChecked">
                    {{ __('components.tree.filament-tree.components.tree.buttons.deselect_all.label') }}
                </span>
                <span x-show="!areAllCheckboxesChecked">
                    {{ __('components.tree.filament-tree.components.tree.buttons.select_all.label') }}
                </span>
            </x-filament::link>

            <x-filament::icon-button size="sm" icon="heroicon-o-plus" color="secondary"
                label="{{ __('components.tree.filament-tree.components.tree.buttons.expand_all.label') }}"
                x-on:click="toggleExpandAll(true)" />

            <x-filament::icon-button size="sm" icon="heroicon-o-minus" color="secondary"
                label="{{ __('components.tree.filament-tree.components.tree.buttons.collapse_all.label') }}"
        ...
        <div>
            @foreach ($getOptions() as $node)
                <x-tree.components.forms.tree.group :node="$node" />
            @endforeach
        </div>
    </div>
</x-dynamic-component>
