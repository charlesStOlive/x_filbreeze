<?php

namespace App\Console\Commands;

use Illuminate\Console\Command;
use Illuminate\Support\Facades\Http;

class ListMsGraphSubscriptions extends Command
{
    protected $signature = 'msgraph:list-subscriptions';
    protected $description = 'List all Microsoft Graph subscriptions';

    public function handle()
    {
        // Obtenir un token d'accès
        $token = $this->getAccessToken();
        if (!$token) {
            $this->error('Failed to get access token');
            return 1;
        }

        // Lister les subscriptions
        $response = Http::withToken($token)
            ->get('https://graph.microsoft.com/v1.0/subscriptions');

        if ($response->successful()) {
            $subscriptions = $response->json()['value'];

            if (empty($subscriptions)) {
                $this->info('No subscriptions found');
                return 0;
            }

            $this->info('Current subscriptions:');
            foreach ($subscriptions as $subscription) {
                $this->info('ID: ' . $subscription['id']);
                $this->info('Resource: ' . $subscription['resource']);
                $this->info('Notification URL: ' . $subscription['notificationUrl']);
                $this->info('Expiration: ' . $subscription['expirationDateTime']);
                $this->info('---');
            }

            return 0;
        } else {
            $this->error('Failed to list subscriptions');
            $this->error('Response: ' . $response->body());
            return 1;
        }
    }

    private function getAccessToken()
    {
        $response = Http::asForm()->post(config('msgraph.tenantUrlAccessToken'), [
            'client_id' => config('msgraph.clientId'),
            'client_secret' => config('msgraph.clientSecret'),
            'scope' => 'https://graph.microsoft.com/.default',
            'grant_type' => 'client_credentials'
        ]);

        if ($response->successful()) {
            return $response->json()['access_token'];
        }

        $this->error('Failed to get access token: ' . $response->body());
        return null;
    }
}
